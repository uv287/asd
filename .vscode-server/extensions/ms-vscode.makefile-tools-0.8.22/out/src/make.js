...
function getIsPostConfiguring() { return isPostConfiguring; }
exports.getIsPostConfiguring = getIsPostConfiguring;
;
function setIsPostConfiguring(postConfiguring) { isPostConfiguring = postConfiguring; }
    ...
    try {
        return await vscode.window.withProgress({
            ...
            cancel.onCancellationRequested(async () => {
                progress.report({ increment: 1, message: localize(6, null) });
...
async function prePostConfigureHelper(titles, configureScriptMethod, setConfigureScriptState, logConfigureScriptTelemetry) {
    ...
    try {
        return await vscode.window.withProgress({
            ...
            cancel.onCancellationRequested(async () => {
                progress.report({ increment: 1, message: localize(9, null) });
...
async function preConfigure(triggeredBy) {
    ...
    return await prePostConfigureHelper({ configuringScript: localize(10, null, scriptFile), cancelling: localize(11, null) }, (progress) => runPreConfigureScript(progress, scriptFile), (value) => setIsPreConfiguring(value), (elapsedTime, exitCode) => {
        const telemetryMeasures = {
...
async function postConfigure(triggeredBy) {
    ...
    // Assert that scriptFile is not undefined at this point since we've checked above.
    return await prePostConfigureHelper({ configuringScript: localize(12, null, scriptFile), cancelling: localize(13, null) }, (progress) => runPostConfigureScript(progress, scriptFile), (value) => setIsPostConfiguring(value), (elapsedTime, exitCode) => {
        const telemetryMeasures = {
...
async function loadConfigurationFromCache(progress, cancel) {
    ...
    if (cachePath) {
        ...
        if (content) {
            try {
                progress.report({ increment: 1, message: localize(19, null) });
